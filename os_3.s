.data
taskA_stack:  .space 512
taskA_uepc:   .word  0
taskB_stack:  .space 512
taskB_uepc:   .word  0

current_task: .word 0   # checks which task is running (0 for taskA and 1 for taskB)
...
# save the context for the current task
  beqz t0, save_taskA_context
  b    save_taskB_context

# save the context for taskA
save_taskA_context:
  csrrw t1, uepc, zero  # get the pc for taskA
  la    t2, taskA_uepc
  sw    t1, 0(t2)           
  mv    t1, sp
  la    t2, taskA_stack
...
# save the context for taskB 
save_taskB_context:
  csrrw  t1, uepc, zero  # get the pc for taskB
  la     t2, taskB_uepc
  sw     t1, 0(t2)           
  mv     t1, sp
  la     t2, taskB_stack
...
switch_task:
  ...
  xori   t0, t0, 1            
  sw     t0, 0(t1)
  beqz   t0, restore_taskA_context
  b      restore_taskB_context
  
restore_taskA_context:
  la     t2, taskA_uepc
  lw     t1, 0(t2)
  csrrw  zero, uepc, t1      
  la     t2, taskA_stack
...
# intialise the stacks for taskA
  la     t0, taskA_stack
  addi   t0, t0, 512  # move to top of taskA stack
  la     t1, taskA_stack
  sw     t0, 0(t1)
# intialise the stacks for taskB
  la     t0, taskB_stack
  addi   t0, t0, 512  # move to top of taskB stack
  la     t1, taskB_stack
  sw     t0, 0(t1)
# start with taskB
  li     t0, 1
...
# read the current value of the timer counter
  li     s0,TIME   
  lw     t1, 0(s0)
# add 100 milliseconds to the timer counter value
  addi   t1,t1,100 
# store the updated value to the timecmp register
  li     s1,TIMECMP    
  sw     t1, 0(s1)  
...
# entry point for taskA
  la     t0, taskA       
  la     t1, taskA_uepc
  sw     t0, 0(t1)
# entry point for taskB
  la     t0, taskB
  la     t1, taskB_uepc
  sw     t0, 0(t1)
